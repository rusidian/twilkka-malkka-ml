...
if not DATA_PREPROCESS_ROOT.exists():
    raise FileNotFoundError(f"data_processor.py 폴더를 찾을 수 없습니다: {DATA_PREPROCESS_ROOT}")

if str(DATA_PREPROCESS_ROOT) not in sys.path:
    sys.path.append(str(DATA_PREPROCESS_ROOT))

from data_processor import DataProcessor
from upload_service import load_uploaded_dataframe
...
def prepare_inference_dataframe(uploaded_file) -> pd.DataFrame:
    raw_user_df = load_uploaded_dataframe(uploaded_file)

    processor = DataProcessor(config=None)
    inference_df = processor.run_full_pipeline(raw_user_df)
...
def build_analysis_payload(uploaded_file, model_path: str, uploaded_file_name: str | None = None) -> dict:
    ...
    final_file_name = uploaded_file_name or getattr(uploaded_file, "name", "uploaded_file")

    return {
        "headline_insight": make_headline_insight(result_df, importance_df),
        "data_meta": make_data_meta(result_df, final_file_name),
